...
Now we have the chat screen working , user can also login , lets have some attendees and after the user logs-in they will view all attendees in the event  `attendees_screen`, => and attendees can chat among themselves.
...
modify **main.dart** 📄  home: and add routes for navigation -> we create function **_getScreenId** in the next step
...
We got all our references now we need to navigate from main to attendees screen if we are logged in. update function _getScreenId() in  main.dart 📄 file 
...
```
return AllAttendees(appUsers: appUsers);
```

# User profile images

Profile images will come from the network, We will use Flutter library to load and cache network images We are using cached_network_image ,add to pubspec.yaml 📄 file and get the package.
...
Finally add a new screen to edit user profile and upload/edit profile image for that first we need a service to upload image to **firebase storage** see next section =>